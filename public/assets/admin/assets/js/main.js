...
// Set sidebar to active if matches the href
let sidebarItem = document.querySelectorAll('.sidebar-item')
for (let i = 0; i < sidebarItem.length; i++) {
  if (sidebarItem[i].firstElementChild.getAttribute('href') == window.location.href) {
    sidebarItem[i].classList.add('active')
  }
  if (sidebarItem[i].classList.contains('has-sub')) {
    let submenu = sidebarItem[i].lastElementChild;
    let submenuChildren = sidebarItem[i].lastElementChild.children;
    
    for (let j = 0; j < submenuChildren.length; j++) {
      if (submenuChildren[j].firstElementChild.getAttribute('href') == window.location.href) {
        sidebarItem[i].classList.add('active')
